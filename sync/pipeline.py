...
1. 第 2 步：按 清单.md 的排位写好 单套任务.json，用 skill 里的 gen_parallel.py 把 9 张一次全发出去（默认 9 张同时出），到 单套/01.png…09.png（每张用对应原图当参考）。出完统一检查：逐张和原图对比，严格按「衣服细节一致」检查，不合格的只用 gen.py 重跑那张，每张最多重跑 2 次。
2. 第 3 步：拼 封面.jpg。
